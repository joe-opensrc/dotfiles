...
function __PSC_git() {
  ...
  local ahead=$( git status -sb 2>/dev/null | grep '^##' | grep 'origin' | grep '\[ahead [0-9]\+\]' )
  if [[ "x${ahead}" != "x" ]]
  then 
    echo -n "[GitPush!!]"
  fi
